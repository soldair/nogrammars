...
Game.prototype = {
	...
	heartBeat:function(){
		...
		setInterval(function(){
			z.beatCbs.forEach(function(cb,k){
				...
				//TODO MIGRATE TO REGISTER METHOD
				if(changes && changes.length){
					if(!z.changes) z.changes = {};
					if(!z.changes.units) z.changes.units = [];
					z.changes.units.push.apply(z.changes,changes);
				}
			});
			//changes structure should probably support delete
			z.pushChanges();
			z.pushDeletes();
		},this.gameState.serverInterval);
	},
	...
	//TODO do i have to create players as game objects?
	...
	assignPlayerToTeam:function(id){
		...
		if(this.teamCount < this.lastTeamAssigned) {
	...
	moveUnit:function(unit,destination){
		if(unit){
			...
			//TODO apply server side movement
